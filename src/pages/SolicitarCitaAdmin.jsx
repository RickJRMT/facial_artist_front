...
const SolicitarCitaAdmin = ({ initialData = null, esAdmin = false, onCitaCreada, onSuccess, onClose }) => {
    ...
    useEffect(() => {
        if (esEdicion && initialData) {
            ...
            // ← CORREGIDO: Prefill correcto de estados (con fallback seguro)
    ...
    // Ajuste automático de hora si no coincide
    useEffect(() => {
    ...
    const manejarEnvio = async (e) => {
        ...
        if (!esAdmin && !esEdicion) {
            // ... tus validaciones de edad, nombre, etc. (sin cambios)
        ...
        const datosCita = {
            ...
            estadoCita,     // ← Ahora sí se envía correctamente
            estadoPago      // ← Ahora sí se envía correctamente
        };
        ...
        try {
            ...
            if (esEdicion) {
                ...
                // Modal bonito SIN alert del navegador
                onSuccess({
                    tipo: "editada",
    ...
    return (
        <>
            <div className="solicitar-cita-card">
                ...
                {loading && esEdicion && <p>Cargando datos...</p>}

                <form className="solicitar-cita-card__form" onSubmit={manejarEnvio}>
                    {/* COLUMNA IZQUIERDA */}
                    <div className="solicitar-cita-card__column">
                        <label>Nombre Completo</label>
                        <input type="text" name="nombreCliente" value={formData.nombreCliente} onChange={handleInputChange} required disabled={esEdicion && esAdmin} />

                        <label>Teléfono</label>
                        <input type="text" name="celularCliente" value={formData.celularCliente} onChange={handleInputChange} required disabled={esEdicion && esAdmin} />

                        <label>Fecha de Nacimiento</label>
                        <input type="date" name="fechaNacCliente" value={formData.fechaNacCliente} onChange={handleInputChange} max={new Date().toISOString().split("T")[0]} required disabled={esEdicion && esAdmin} />

                        <label>Servicio</label>
                        <select value={idServicio} onChange={handleServicioChange} required disabled={esEdicion}>
                    ...
                    <div className="solicitar-cita-card__column">
                        ...
                        <select value={idProfesional} onChange={handleProfesionalChange} required>
                        ...
                        <input type="date" name="fechaCita" value={formData.fechaCita || ""} onChange={handleInputChange} min={new Date().toISOString().split("T")[0]} required />

                        <label>Hora para la cita</label>
                        <select name="horaCita" value={formData.horaCita || ""} onChange={handleInputChange} required disabled={!idProfesional || !idServicio || !formData.fechaCita || (horariosDisponibles.length === 0 && !esEdicion)}>
                        ...
                        {esEdicion && (
                            <>
                                ...
                                <select value={estadoCita} onChange={(e) => setEstadoCita(e.target.value)} required>
                                ...
                                <select value={estadoPago} onChange={(e) => setEstadoPago(e.target.value)} required>
                                    <option value="pendiente">Pendiente</option>
